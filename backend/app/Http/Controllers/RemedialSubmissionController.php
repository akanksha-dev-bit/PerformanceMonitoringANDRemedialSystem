...
{
    // ── Security helper: abort if submission's school != user's school ──────
    ...
    // ── Student: view submission workspace ───────────────────────────────────
    public function show(RemedialAction $remedial)
    {
        ...
        // Ensure this action belongs to this student
    ...
    // ── Student: autosave draft (called every 30s) ───────────────────────────
    public function saveDraft(Request $request, RemedialAction $remedial)
    {
        ...
        $submission = RemedialSubmission::where('remedial_action_id', $remedial->id)
            ...
            ->where('school_id', $user->school_id) // cross-school check
            ->firstOrFail();
    ...
    public function submit(Request $request, RemedialAction $remedial)
    {
        ...
        if (!$submission->can_edit) {
            return back()->with('error', 'Your submission has already been reviewed.');
        }
    ...
    public function uploadFile(Request $request, RemedialAction $remedial)
    {
        ...
        // File validation — only safe types, max 10MB
        $request->validate([
    ...
    public function grade(Request $request, RemedialSubmission $submission)
    {
        ...
        // Mark remedial action as completed
    ...
    // ── Teacher: reopen / request resubmission ───────────────────────────────
